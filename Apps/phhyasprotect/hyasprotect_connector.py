...
class HyasProtectConnector(BaseConnector):
    ...
    def _validating_ioc(self, action_result, ioc, val):
        try:
            if ioc in IOC_NAME:
                if ioc == 'ip':
                    return bool(re.fullmatch(IOC_NAME[ioc], val)) or bool(re.fullmatch(IPV6_REG, val))
                else:
    ...
    def _make_rest_call(
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR, "Handled exception: {0}".format(error_message)
                ),
                None,
            )
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR, "Error connecting: {0}".format(error_message)
                ),
                None,
            )
